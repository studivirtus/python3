...
class iterativeDeepeningSearch():
    ...
    def __init__(self,firstNode,goalValue):
        self.addToFrontier(firstNode)
    ...
    def explore(self):
        ...
        while((not self.frontierIsEmpty()) and (not found)):
            while((not self.frontierIsEmpty()) and (not found)):
                ...
                if self.goalReached(curNode.nValue):
                    ...
                    break
                self.addNodesVisited(curNode)
                if not self.atMaxDepth(curNode):
        ...
        if(found):
            print("Iterative Deepening search FOUND A SOLUTION %i" % solutionNode.nValue)
            print("here are the nodes visited:")
            self.printlist(self.nodesVisited)
        elif not found and self.frontierIsEmpty():
            print("FAILURE, Iterative Deepening search did not find a solution")
            print("here are the nodes visited:")
            self.printlist(self.nodesVisited)
...
if __name__ == '__main__':
    ...
    ITS = iterativeDeepeningSearch(startingNode,goalNode)
    ITS.explore()
